...
- **TODO (kernel):** track down the 7.0.0-kernel #GP (suspect CET/IBT + KVM guest state); until then the `-4` workaround is the safe default. If IPv6 is ever needed on the enclave, the watchdog alone is not sufficient (a re-run that hits the same parse path can fault) — fix the kernel first.
