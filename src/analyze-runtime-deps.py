...
def generate_package_run_blocks(package_deps: Dict[str, Set[str]]):
    ...
    for cf_path in containerfiles:
        ...
        # Insert package-run block AFTER each "FROM ... AS package-*" block
        ...
        while i < len(lines):
            ...
            if match:
                ...
                # Collect the entire package block (FROM + subsequent COPY lines)
                block = [line]
                i += 1
                ...
                # Append package-run block after the package block
                if deps:
                    new_lines.append("")
                    new_lines.append(f"FROM {package_stage} AS {package_stage}-run")
