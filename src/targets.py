        ...
        print(
          TargetGenerator.TARGET_TEMPLATE.format(
            **{
              ...
              "files": "\\\n\t".join(line.replace(" ", "\\ ") for line in check_output(["find","packages/{}/{}".format(stage,package.origin or package.name),"-type","f","-not","-path","*/fetch/*","-not","-name","provenance.spdx"],text=True).splitlines()),
